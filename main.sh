...
if [ "$ask" == "Update -- Experimental" ]; then
  qterminal --workdir ~ --execute "lxsudo apt update; lxsudo apt upgrade; lxsudo pip install --upgrade pip; lxsudo pip install mitmproxy; lxsudo npm install -g apk-mitm"
fi
